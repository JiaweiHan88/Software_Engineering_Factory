...
{if-sidecar}
## Sidecar
Memory location: `bmad_res/_memory/{skillName}-sidecar/`
...
{/if-no-autonomous}
   {if-sidecar}- **Check first-run** — If no `{skillName}-sidecar/` folder exists in `bmad_res/_memory/`, load `init.md` for first-run setup
   - **Load access boundaries** — Read `bmad_res/_memory/{skillName}-sidecar/access-boundaries.md` to enforce read/write/deny zones (load before any file operations)
   - **Load memory** — Read `bmad_res/_memory/{skillName}-sidecar/index.md` for essential context and previous session{/if-sidecar}
