...
5. No absolute paths
6. Memory paths must use {project-root}/bmad_res/_memory/{skillName}-sidecar/
"""
...
# Memory path pattern: should use {project-root}/bmad_res/_memory/
MEMORY_PATH_RE = re.compile(r'bmad_res/_memory/\S+')
VALID_MEMORY_PATH_RE = re.compile(r'\{project-root\}/bmad_res/_memory/\S+-sidecar/')
...
def scan_file(filepath: Path, skip_fenced: bool = True) -> list[dict]:
    ...
    checks = [
        (PROJECT_ROOT_NOT_BMAD_RE, 'project-root-not-bmad', 'critical',
         '{project-root} used for non-_bmad path — only valid use is {project-root}/bmad_res/...'),
    ...
    # Memory path check — memory paths should use {project-root}/bmad_res/_memory/{skillName}-sidecar/
    for match in MEMORY_PATH_RE.finditer(content):
        ...
        if '{project-root}/' not in before:
            ...
            findings.append({
                ...
                'title': 'Memory path missing {project-root} prefix — use {project-root}/bmad_res/_memory/',
                'detail': line_content[:120],
                'action': '',
            })
