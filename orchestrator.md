...
After setup, a cron job fires every <!--CONFIG:CRON_INTERVAL-->3<!--/CONFIG:CRON_INTERVAL--> minutes with a **self-contained systemEvent** that has everything you need — you won't need to reference Part 1 or Part 2 during cron execution.
...
# ============================================================
# Generated by: The Claw Loop v2.3.1 — Autonomous Dev Orchestrator
#
# TIER DEFINITIONS:
...
Create a cron called `bmad-dev-loop` that fires every **<!--CONFIG:CRON_INTERVAL-->3<!--/CONFIG:CRON_INTERVAL--> minutes** (fixed interval, never changes) with the systemEvent text from the **CRON SYSTEMEVENT** section below.

⚠️ **Copy the ENTIRE systemEvent block exactly as written. It is self-contained and must not be edited or abbreviated.**

## Step 8: Create the Daily Summary Cron
...
## Step 9: Add Watchdog to Persistent Memory
...
```markdown
## Active Automations
- **Claw Loop v2.3.1** is active for project [PROJECT_NAME] at [PROJECT_PATH]. State file: memory/bmad-dev-state.json. Activity log: [PROJECT_PATH]/_bmad-output/implementation-artifacts/claw-loop-activity.log. If status is "running", the bmad-dev-loop cron should be firing every <!--CONFIG:CRON_INTERVAL-->3<!--/CONFIG:CRON_INTERVAL--> minutes. See HEARTBEAT.md for watchdog instructions.
```
...
☐ Tell the human: "Claw Loop v2.3.1 is live. Fixed <!--CONFIG:CRON_INTERVAL-->3<!--/CONFIG:CRON_INTERVAL-->-min cron with smart-skip on [channel]. Starting: Story [X.X] [name] (model: [tier] → [model]). Queue: [N] stories across [M] epics. Model strategy: [X] epics highest, [Y] epics standard. Cron watchdog active. Say 'pause loop' anytime to stop."

---

# CRON SYSTEMEVENT — Self-Contained Execution Block

**This is the text that goes into the `bmad-dev-loop` cron's systemEvent field.**
**It is fully self-contained. The Clawdbot reads ONLY this text every <!--CONFIG:CRON_INTERVAL-->3<!--/CONFIG:CRON_INTERVAL--> minutes — no other context is available.**
...
☐ 5c. Send report to human on [CHANNEL] (target: [TARGET_ID]):
    ...
    → Calculate elapsed: NOW - state.metrics.currentStoryStartedAt (round to nearest minute)
    ...
    **[WORKING]** (CC active, no intervention):
    ...
    ACTION: None — <brief reason>
    PROGRESS: Story <N>/<Total> | Epic <N> | Reviews: <N>
    ```
    ...
    **[DONE]** (story complete — only on STORY_DONE):
    ...
    ACTION: Advancing to story Y.Y
    PROGRESS: Story <N>/<Total> complete | Epic <N> (<M> remaining)
    STATS: <elapsed> min | <N> cycles | <N> review passes | <N> review passes | <model tier>
    ```
...
| Feature | v1.2 | v2.2 | v2.3 |
|---------|------|------|------|
| Document structure | Single flow | Single flow | CONCEPTS + EXECUTE + self-contained CRON |
| Cron systemEvent | Cross-references setup | Cross-references setup | Fully self-contained, zero external refs |
| Critical rules | End of document | End of document | TOP of cron as immutable preamble |
| Cycle verification | None | None | End-of-cycle checklist with ☐ markers |
| Decision format | Prose paragraphs | Prose paragraphs | IF/THEN/ACTION/STATE blocks |
| Structural markers | Markdown only | Markdown only | XML tags + severity markers (⛔/⚠️/ℹ️) |
| Orphaned rules | Items 5-13 at bottom | Items 5-13 at bottom | Merged into procedural steps |
| Formatting | Mixed patterns | Mixed patterns | Standardized: code fences / bold / → arrows |
| Model selection | Hardcoded | Pre-computed from YAML | Pre-computed + validate-on-read + fallback |
| Unrecognized prompts | Send `c` blindly | Two-pass protocol | Two-pass (inline in cron, no cross-ref) |
| Git safety | None | Commit verification | Commit verification (inline in cron) |
| Daily rollup | None | Separate cron | Separate cron |
| Quarantine | None | Quarantine command | Quarantine command |
| Stall fingerprinting | Raw hash | Semantic (noise stripped) | Semantic (inline in cron) |
